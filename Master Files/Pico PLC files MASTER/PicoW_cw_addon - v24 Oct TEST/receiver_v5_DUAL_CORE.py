# reciever_v5_DUAL_CORE.py - from OpenPLC using v23, for displaying data via thread. now with logo graphic
# cw error count (use hidden button)
...
while True:
    ...
    if button_a.read(): # lets us view the number of com's errors
        display.clear()
        display.set_pen(BLUE)
        display.text(str(eCount), 100, 80, 200, 3)
        display.update()
        
    display.text("DC OpenPLC", 70, 20, 200, 3)
    display.text("(v23)", 230, 20, 230, 1)
